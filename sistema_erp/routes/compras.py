...
def salvar_entrada():
    ...
    origem_novo = request.form.get('origem_novo', '0') # Nova Origem
    ...
    creditos = 0.0
    if lucro_real: creditos = val_icms + val_pis + val_cofins
    ...
    if check_novo == 'on' or (nome_novo and nome_novo.strip() != ''):
        if not nome_novo:
            return jsonify({'success': False, 'message': 'Nome do produto obrigatório.'})
        ...
        try: produto_id = int(produto_id_raw)
        except: return jsonify({'success': False, 'message': 'Selecione um produto válido.'})
    ...
    # Atualiza Produto (Agora incluindo Fornecedor se mudar)
    sql_update = """
        UPDATE produtos 
    ...
    return jsonify({'success': True, 'message': f'Entrada confirmada! Custo: R$ {custo_final:.2f}'})